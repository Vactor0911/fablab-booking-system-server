...
// csrf 토큰 확인
app.post("/csrf-check", csrfProtection, (req: Request, res: Response) => {
...
app.post("/users/login", csrfProtection, (req: Request, res: Response) => {
  ...
  db.query("SELECT id, state FROM user WHERE id = ?", [id])
    .then((rows: any) => {
      ...
      // Step 1: ID로 사용자 조회
      return db.query("SELECT * FROM user WHERE id = ? AND state = 'active'", [
        id,
      ]);
    })
    .then((rows: any) => {
        ...
        return db
          ...
          .then(() => {
            ...
            res.cookie("refreshToken", refreshToken, {
              httpOnly: true,
              secure: true, // ture : HTTPS 환경에서만 작동, false : HTTP 환경에서도 작동(로컬 환경)- refreshToken만 ture로 설정
              sameSite: "none", // CSRF 방지 -> 이거 사용보다 그냥 CSRF 토큰 사용하는게 더 안전 -> 따라서 적용
  ...
  (req: Request, res: Response) => {
    ...
    db.query("SELECT id, state FROM user WHERE id = ?", [id])
      ...
      .then((rows: any) => {
        ...
        // Step 2: 사용자 ID 중복 확인
        return db.query(
          "SELECT id FROM user WHERE id = ? AND state = 'active'",
          [id]
        );
      })
      .then((rows: any) => {
...
app.post("/users/logout", csrfProtection, (req: Request, res: Response) => {
  ...
  db.query("SELECT * FROM user WHERE refreshtoken = ?", [refreshToken])
    .then((rows: any[]) => {
      ...
      return db
        .query("UPDATE user SET refreshtoken = NULL WHERE refreshtoken = ?", [
          refreshToken,
        ])
        .then(() => {
          // 클라이언트에서 쿠키 삭제
...
app.delete(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      await connection.query(
        "UPDATE book SET state = 'end' WHERE user_id = ? AND state = 'book'",
        [userId]
      );
  ...
  (req: Request, res: Response) => {
    ...
    db.query("SELECT * FROM user WHERE id = ? AND email = ?", [id, email])
      .then((rows: any[]) => {
        ...
        // Step 2: 계정 상태 복구
        return db.query(
          "UPDATE user SET state = 'active' WHERE id = ? AND email = ?",
          [id, email]
        );
      })
      .then(() => {
...
app.get(
  ...
  (req: Request, res: Response) => {
    ...
    // DB에서 사용자 정보 조회
    db.query("SELECT id, name, email, permission FROM user WHERE user_id = ?", [
      userId,
    ])
      .then((rows: any[]) => {
        if (rows.length === 0) {
...
app.get("/notice/:uuid", async (req: Request, res: Response) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "공지사항 데이터를 가져오는 중 오류가 발생했습니다." });
  }
});
// 공지사항 내용 조회 API 끝
...
app.patch(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ message: "조회수 증가 처리 중 오류가 발생했습니다." });
    }
  }
);
// 공지사항 조회수 증가 API 끝
